...
try {
    ...
    $counselor_id = authenticate_user_from_jwt($pdo, true); // THIS LINE returns an INT (the counselor_id)
    ...
    // This section is now CORRECTLY checking the result of the DB query
    if (!$counselor_data || empty($counselor_data['email'])) {
        ...
        echo json_encode(["success" => false, "error" => "Counselor email not found in authentication data."]);
        exit();
    }
